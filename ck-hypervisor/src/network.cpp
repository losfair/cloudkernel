...
static std::optional<IPAddress> extract_destination_address_from_packet(const uint8_t *data, size_t len) {
...
void Router::register_route(IPAddress addr, std::shared_ptr<RoutingEndpoint>&& endpoint) {
...
void Router::unregister_route(IPAddress addr, __uint128_t ck_pid) {
    std::lock_guard<std::mutex> lg(mu);
    if(auto it = routes.find(addr); it != routes.end() && it->second->ck_pid == ck_pid) {
        routes.erase(it);
    }
}
...
void Router::dispatch_packet(uint8_t *data, size_t len) {
    ...
    if(endpoint) {
        ...
        if(endpoint->on_packet) endpoint->on_packet(data, len);
    } else {
...
int Tun::write(uint8_t *packet, size_t len) {
    return ::write(fd, packet, len);
}

int Tun::read(uint8_t *packet, size_t len) {
