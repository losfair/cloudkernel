...
static ModuleHandle * try_open_module(const char *prefix, const char *name, VersionCode version) {
    {
...
void Registry::save_module(const char *name, std::optional<VersionCode> version, const char *suffix, const uint8_t *code, size_t code_len) {
    if(!validate_module_name(name)) throw std::runtime_error("invalid module name");
    std::lock_guard<std::mutex> lg(this->mu);
    ...
    if(version) {
        filename_builder << "_" << version->major << "." << version->minor << "." << version->patch;
    }
    ...
    int fd = open(path_s_tmp.c_str(), O_WRONLY | O_CREAT | O_CLOEXEC, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    if(fd < 0) {
