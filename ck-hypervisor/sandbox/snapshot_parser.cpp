...
static long __attribute__((naked)) enable_notify_invalid_syscall() {
  asm("movq $" _STR(CK_SYS_NOTIFY_INVALID_SYSCALL) ", %rax\n"
                                                   "syscall\n"
                                                   "ret\n");
}

int __attribute__((noinline))
load_snapshot(int mfd, const uint8_t *snapshot, size_t len,
  ...
  for (uint32_t i = 0; i < n_memory_ranges; i++) {
    ...
    case MemoryRangeType::DATA: {
      void *ptr = mmap((void *)start_addr, data_len, prot,
                       MAP_PRIVATE | MAP_FIXED, mfd, data_offset);
      if (ptr == MAP_FAILED) {
    ...
    case MemoryRangeType::STACK: {
      ...
      // touch stack
      for (uint64_t *p = ((uint64_t *)stack_end) - 1;
           p >= (uint64_t *)start_addr; p--) {
        *p = 0;
      }
      std::copy(snapshot + data_offset, snapshot + data_offset + data_len,
                (uint8_t *)start_addr);
      break;
    }
    default:
      throw std::runtime_error("unknown memory range type");
    }
  }
  ...
  for (uint32_t i = 0; i < n_files; i++) {
    ...
    std::string path;
    read_vec_n(snapshot, len, pos, path, path_len);
